...
object Team {
  ...
  implicit val jsonDecoder: Decoder[Team] = cursor =>
    ...
    } yield Team(Id(id), Name(name), Point(points),Transfer(transfers))
  ...
  implicit val teamRead: Read[Team] = Read[(Int, String, Int, Int)].map {
    case (id, name, points, transfers_amount) => Team(Id(id), Name(name), Point(points), Transfer(transfers_amount))
  }
